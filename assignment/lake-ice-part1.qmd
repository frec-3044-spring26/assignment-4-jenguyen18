...
p <- sunapee |>
  ...
  geom_point() +
  geom_line() +
  labs(title = "Lake Sunapee Ice Off",
       x = "Year",
       y = "Ice Off Doy (Day of Year)") +
  theme_classic()
p
```
...
**Answer 2:**
I expect the sign to be negative because as the year increased the amount of ice off days from 70-90's increases.
...
**Answer 3:**
The direction was negative, which matched my expectations.
...
**Answer 4:**
 The R2 is 0.2055. Year and ice_off_doy are not strongly related.
...
**Answer 6:**
Yes, the residuals meet the assumptions since the residuals are normally distrubed and centered around 0.
```{r}
ggplot(sunapee, aes(x = resid)) +
  geom_histogram(width = 10)
```
...
**Answer 8:**
7/.1060227 = 66.02. It would take 66 years to pass. I think it's meaningful because of the low p-value.
...
**Answer 9:**
I would be comfortable even though r-sqaured is low, atleast the p-value (< .05) and residuals are what we would want.
**Question 10:**
...
**Answer 10:**
```{r}
# p + geom_abline(mapping = NULL,
 # data = NULL, slope = -.1060227, intercept = 318.9486,
 #               color = "blue",
  #na.rm = FALSE)
...
sun_late <- sunapee |>
  filter(year > 1970)
p1 <- ggplot(sun_early, aes(x = year, y = ice_off_doy)) +
  geom_point() +
   labs(title = "Lake Sunapee Ice Off",
       x = "Year",
       y = "Ice Off Doy (Day of Year)") +
  theme_classic()
p2 <- ggplot(sun_late, aes(x = year, y = ice_off_doy)) +
  geom_point() + 
   labs(title = "Lake Sunapee Ice Off",
       x = "Year",
       y = "Ice Off Doy (Day of Year)") +
...
fit_early = lm(ice_off_doy ~ year, data = sun_early)
fit_late = lm(ice_off_doy ~ year, data = sun_late)
summary(fit_early)
summary(fit_late)
#fit_early + geom_abline(slope = -.29271, intercept = 692)
```
