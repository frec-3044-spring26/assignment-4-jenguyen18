...
```{r}
library(tidyverse)
sunapee <- filter(data, lake_name == "Sunapee")
```
...
```{r}
fit = lm(year~ice_off_doy , data = sunapee)
```
...
**Answer 4:**
 The R2 is .4457. Year and ice_off_doy are not strongly related.
...
library(modelr)
sunapee <- sunapee |>
add_predictions(fit, var = "pred", type = NULL) |>
...
**Answer 6:**
Yes, the residuals meet the assumptions.
```{r}
